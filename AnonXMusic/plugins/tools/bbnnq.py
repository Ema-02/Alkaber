...
@app.on_message(command("سيلا") & filters.group)
async def bottttt(client, message):
    selections = [f"عمرها لـ سيلا 🤍🧚🏼‍♂️ {message.from_user.mention}", 
f"يا قلب سيلا ♥ {message.from_user.mention}",
f"صرعت راسها لـ سيلا 🙈 {message.from_user.mention}",
...
f"تفضل واطلب ايدي 🧚🏼‍♂️ {message.from_user.mention}",
...
async def ahmad(client: Client, message: Message):
    await message.reply_text(f"""🧚🏼‍♂️🔥 أوامࢪ بوت سيلا 🧚🏼‍♂️🔥:
...
‹: سيلا + السؤال (فقط في الخاص) 🥀
‹: غنيلي - سترسل لك اغنية عشوائية 🥀
""",
        reply_markup=InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("‹ اضافة الى مجموعة ›", url=f"https://t.me/Musoxbot?startgroup=true"),
            ],
            ]
        ),
    )